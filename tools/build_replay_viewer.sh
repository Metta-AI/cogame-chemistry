...
# `coworld build` pre-creates the output's parent; CI does not, and the
# containment check below resolves it by cd-ing into it. Create it FIRST --
# every fork of paintbot's hook inherits this bug (ecos, 2026-08-23).
...
image_tag="coworld-chemistry-replay-viewer-build:$$"
container_id=""
cleanup() {
...
docker cp "${container_id}:/workspace/chemistry/replay-viewer/dist/." "${output_dir}"
