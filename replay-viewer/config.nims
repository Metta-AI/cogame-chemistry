...
# use-after-free fixed in chemistry_replay.nim (emscripten_exit_with_live_runtime)
...
# chemistry_error_len(). Aborting keeps linear memory intact, and the page reads
# chemistry_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "chemistry_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_chemistry_load_replay,_chemistry_frame,_chemistry_input,_chemistry_packet_ptr,_chemistry_packet_len,_chemistry_error_ptr,_chemistry_error_len,_chemistry_stage_ptr,_chemistry_stage_len
  """).replace("\n", " ")
)
